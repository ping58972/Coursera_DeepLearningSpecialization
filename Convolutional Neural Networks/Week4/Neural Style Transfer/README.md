...
def compute_content_cost(a_C, a_G):
    ...
    J_content = tf.divide(tf.reduce_sum(tf.square(tf.subtract(a_G_unrolled,a_C_unrolled))),4*n_H*n_W*n_C)
    ### END CODE HERE ###
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.divide(tf.reduce_sum(tf.square(tf.subtract(GS,GG))),4*(n_C**2)*((n_W*n_H)**2))
...
```
    ...
    Iteration 0 :
    total cost = 5.85675e+09
    content cost = 7689.74
    style cost = 1.46417e+08
    ...
    <ipython-input-36-aed364014da4> in <module>()
    ----> 1 model_nn(sess, generated_image)
    

    <ipython-input-35-9abff55c73d2> in model_nn(sess, input_image, num_iterations)
